...
const squares = [];
const anchorColors = [
  new THREE.Color(0x5130a9),
  new THREE.Color(0xd08fef),
  new THREE.Color(0x18b08a),
  new THREE.Color(0xfffca9)
];
...
renderer.domElement.addEventListener('mousemove', event => {
  ...
  squares.forEach(square => {
    ...
    // Set the color of the square based on the distance and the anchor colors
    let colorValue;
    if (distance < 10) {
      colorValue = Math.max(0, Math.min(1, distance / 10));
      const color = new THREE.Color().lerpColors(anchorColors[1], anchorColors[0], colorValue);
      square.material.color = color;
    } else {
      colorValue = Math.max(0, Math.min(1, (distance - 10) / 10));
      const color = new THREE.Color().lerpColors(anchorColors[2], anchorColors[3], colorValue);
      square.material.color = color;
    }
